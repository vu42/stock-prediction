...
def create_feature_matrix(data: np.ndarray, seq_length: int) -> tuple[np.ndarray, np.ndarray]:
    """
...
def train_prediction_model(
    ...
    try:
        ...
        if len(df) < settings.sequence_length + 200:
            logger.warning(f"[{stock_symbol}] WARNING: Insufficient data ({len(df)} records)")
            return False
        ...
        feature_columns = [
            ...
            "momentum", "ROC",
            "volatility",
            "SMA_cross_5_20", "SMA_cross_20_60",
        ]
        ...
        if continue_training and os.path.exists(paths["scaler"]):
            try:
                ...
                logger.warning(f"[{stock_symbol}] Could not load scaler: {e}, creating new one")
                scaler = None
        ...
        if continue_training and os.path.exists(paths["model"]):
            try:
                logger.info(f"[{stock_symbol}] Loading existing models for incremental training...")
...
def evaluate_model(db: Session, stock_symbol: str) -> bool:
    ...
    try:
        ...
        ax1.text(
            0.02, 0.98, textstr, transform=ax1.transAxes, fontsize=10,
...
def predict_future_prices(
    ...
    try:
        ...
        ax.plot(
            ...
            color="blue", label="Historical Price", linewidth=2
        )
        
        future_x = range(len(historical_data), len(historical_data) + days_ahead)
        ax.plot(
            future_x, predicted_prices, color="red",
            label=f"{days_ahead}-Day Prediction", linewidth=2,
            linestyle="--", marker="o", markersize=4
        )
        
        ax.axvline(
            x=len(historical_data) - 1, color="green",
        ...
        logger.info(f"[{stock_symbol}] Predicted price range: {pred_min:.2f} - {pred_max:.2f}")
        return True
